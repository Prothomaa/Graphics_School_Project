...
struct Material {
    sampler2D diffuse;//sample er diff r specu...era int type; ambient nei karon eta indirect tai..eta important na
    sampler2D specular;
    float shininess;
};
...
vec3 CalcPointLight(Material material, PointLight light, vec3 N, vec3 fragPos, vec3 V)
{
    ...
    vec3 ambient = vec3(texture(material.diffuse, TexCoords)) * light.ambient;//ekhane materialistic property k replace kora hoese ...but MIX function use kore
